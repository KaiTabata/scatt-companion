...
def suspicious_radius_mm() -> float:
    """誤反応 shot 判定距離 (mm) — 現 discipline の値を返す。
    50m=200, AR=25。種目切替で自動追従。
    """
    return T.current().suspicious_radius_mm
...
class S:
    ...
    DEFAULTS = {
        ...
        # thresholds (suspicious/hold/r95 は射撃種目から自動。z-score のみ user 設定)
...
# 起動時に射撃種目を適用 (50m ライフル / 10m エアライフル)
...
def _gr_velocity(pw, t_arr, samples, sr, sessshots):
    pw.clear()
    # 種目に応じて閾値を変える (50m と AR では妥当な範囲が違う)
...
def _gr_r95_bars(pw, t_arr, samples, sr, sessshots):
    ...
    if not recent:
        ...
        return
    disc = T.current()
    good_mm = disc.r95_good_mm
    bad_mm = disc.r95_bad_mm
    for i, val in enumerate(recent):
        is_current = (i == len(recent) - 1)
        # 値で色: 種目別の good/bad 基準で 低い=緑、中=黒、高い=赤
        if val < good_mm:   col = C.ACCENT_G
        elif val >= bad_mm: col = C.ACCENT_R
    ...
    # 種目別 良/要改善 基準線
    pw.addLine(y=good_mm, pen=pg.mkPen(C.ACCENT_G, width=0.6, style=Qt.PenStyle.DotLine))
    pw.addLine(y=bad_mm,  pen=pg.mkPen(C.ACCENT_R, width=0.6, style=Qt.PenStyle.DotLine))
        ...
        for i, s in enumerate(sessshots):
            ...
            for st in stab:
                if st.get("window_s") == win and st.get("r95") is not None:
                    # 他の *_history グラフと揃えて 0-indexed (shot 内連番)
...
def _gr_cant_history(pw, t_arr, samples, sr, sessshots):
    ...
    # cant 列が None ばかり (= 旧 SCATT 8B/sample 形式で cant が欠落) なら案内表示。
    # 「全 0」を旧形式扱いにはしない: 立射で水平を維持できる射手の全 shot がたまたま
    # 0° 付近に集まるケースを誤検出するため。
    if not xs:
...
def _gr_recoil_xy_overlay(pw, t_arr, samples, sr, sessshots):
    ...
    conn.close()
    # settle 半径 (種目別)
    settle_r = T.current().recoil_settle_mm
    theta = np.linspace(0, 2*np.pi, 64)
    pw.plot(settle_r * np.cos(theta), settle_r * np.sin(theta),
...
def _gr_recoil_settle_history(pw, t_arr, samples, sr, sessshots):
    ...
    pw.clear()
    _setup_plot(pw,
                title=f"反動の戻り時間 推移  ({T.current().recoil_settle_mm:g}mm 以内に戻る)",
                x_label="shot 番号 (session 内)",
...
class SeriesPanel(QWidget):
    ...
    def update_series(self, series_no: int, series_shots: list[dict],
        ...
        if threshold_mm is None:
            threshold_mm = suspicious_radius_mm()
        import datetime
        self.header.setText(
...
class ShotsTab(QWidget):
    ...
    def __init__(self):
        ...
        # AR の suspicious=25 が下限を下回らないよう 5mm からに広げる
        self.threshold_spin.setRange(5.0, 1000.0)
...
class RecoilTab(QWidget):
    ...
    # Settle time のしきい値は種目別 (T.current().recoil_settle_mm)。
    RECOIL_METRICS = [
        ("recoil_peak",       "Peak amplitude",          "mm", "low_good",  2),
        ("recoil_settle",     "Settle time",             "s",  "low_good",  2),
    ...
    def __init__(self):
        ...
        # ラベルに表示するしきい値は現 discipline 由来 (50m=5, AR=1)
        self.hero_settle  = self._compact_card(
            f"反動の戻り時間 ({T.current().recoil_settle_mm:g}mm まで)", "秒"
        )
    ...
    def update_session(self, sid: int | None, meta: dict, session_shots: list[dict],
        ...
        rows = [
            ...
            (f"Settle time (<{T.current().recoil_settle_mm:g}mm)",
                                     stats_row(settle_vals)),
...
<li><b>10a</b>(発射前 1 秒)= Σ<sub>k: t[k]∈[t<sub>fire</sub>−1, t<sub>fire</sub>]</sub> 1[r[k] ≤ R<sub>10</sub>] / N<sub>window</sub> × 100 [%]<br>
  &nbsp;&nbsp;R<sub>10</sub> = 5.2mm (50m ライフル) / 0.25mm (10m エアライフル)</li>
...
class SettingsTab(QWidget):
    ...
    def __init__(self):
        ...
        self.cb_discipline.setToolTip(
            "ターゲット幾何と 10a / 10b / 9c / Hold / R95 / Recoil 各判定値を切り替えます。"
            "適用ボタン押下で即時反映されます。"
        )
        ...
        # ========== Thresholds ==========
        # 誤反応 / hold 速度 / R95 良 / R95 要改善 は射撃種目 (Discipline) で
        # 自動的に決まるため Settings からは外している (旧版では UI から
        # 書き込めても実コードで読まれていなかった)。代わりに 射撃種目 を
        # 切り替えると recoil_settle_mm / hold_v_thr_mm_s 等が一括で変わる。
    ...
    def _on_apply(self):
        ...
        # 射撃種目 (即時反映: ターゲット寸法 / fit / 解析リング半径を全 view に反映)
        ...
        if new_disc and new_disc != cur_disc:
            ...
            T.set_current(new_disc)
            self.discipline_changed.emit()
        # 閾値 (z-score 系のみ。他は射撃種目から自動)
...
class TargetTab(QGraphicsView):
    ...
    def _recreate_target(self):
        ...
        r = T.view_radius_mm(d)
        self.fitInView(-r, -r, 2 * r, 2 * r, Qt.AspectRatioMode.KeepAspectRatio)
        # 軌跡再生中のキャッシュも、新寸法の view 内で再描画
        if self._play_samples and not self._play_timer.isActive():
...
class MainWindow(QMainWindow):
    def __init__(self, db_path: str, auto_live: bool = True, initial_trace: int | None = None):
        ...
        self.settings_tab.discipline_changed.connect(self._on_discipline_changed)
    ...
    def _on_session_selected(self, _idx: int):
        ...
        try:
            ...
            row = None
        # 注: discipline はユーザ Settings (rifle_50m / rifle_10m) で固定。
    ...
    def _on_home_start(self, profile_id: str, discipline_key: str, mode_key: str = None):
        ...
        # discipline 変更時はターゲット寸法も再構築 (mini target + Series target)
    ...
    def _on_discipline_changed(self):
        """discipline (rifle_50m / rifle_10m) 切替の即時反映。

        ターゲット系 view は __init__ 時のリング寸法をインスタンス属性に
        キャッシュしているので、_recreate_target() で scene を作り直さないと
        旧寸法のままになる (= 軌跡が同じ mm でも見た目サイズがズレる)。
        """
        try:
            self.dashboard.mini_target._recreate_target()
        except Exception as e:
            LOG.warn(f"mini target recreate failed: {e}")
        # Sessions タブの SeriesPanel は update_session で再生成されるので
        # 現在 session を再表示すれば新しい discipline で全 series target が作られる
        sid = self._current_session_id
        if sid is not None:
            for k in [k for k in self._session_cache
                      if isinstance(k, tuple) and k[0] == sid]:
                del self._session_cache[k]
            self._update_session_views(sid)
        # 現在 trace を再描画 (リング半径ベースの解析 / 10a 等も更新)
        if getattr(self, "_current_trace_dict", None):
            self._apply_trace(self._current_trace_dict)
        try:
            self.status.showMessage(f"射撃種目: {T.current().label}", 3000)
        except Exception:
            pass
