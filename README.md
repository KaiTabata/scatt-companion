...
**紹介ページ**: <https://kaitabata.github.io/scatt-companion/>
...
git clone https://github.com/KaiTabata/scatt-companion.git
cd scatt-companion
make install
```
...
[Apache License 2.0](LICENSE) — 詳細は [NOTICE](NOTICE) および [THIRD-PARTY-LICENSES.md](THIRD-PARTY-LICENSES.md)。

本ソフトは **慶應義塾軆育會射撃部** 内での利用を前提に開発されている。誰でも自由に利用・改変・再配布できるが、機能・初期値・前提条件は同射撃部の実運用に合わせてチューニングされている。

配布バイナリ (DMG / EXE) には PyQt6 (GPL-3.0) / Qt 6 (LGPL-3.0) などのサードパーティを同梱しており、GPL 義務として対応するソースは本リポジトリのバージョンタグから取得できる。
